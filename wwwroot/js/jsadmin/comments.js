document.addEventListener('DOMContentLoaded', function () {
  ...
  const modalCloseButtons = document.querySelectorAll('.modal-close, #cancelDelete, .modal-close-btn');
  ...
  applyReviewFiltersBtn.addEventListener('click', function() {
      ...
      tableRows.forEach(row => {
          if (row.querySelector('td[colspan="7"]')) { // Skip "no data" row
              row.style.display = ''; return;
          }

          const userName = row.cells[0].querySelector('.user-name')?.textContent.toLowerCase() || '';
          const userEmail = row.cells[0].querySelector('.user-email')?.textContent.toLowerCase() || '';
          const contentData = row.cells[1].querySelector('.comment-content')?.textContent.toLowerCase() || '';
          const rowRating = row.dataset.rating;

          let show = true;

          // Search text filter
          if (searchText && !(userName.includes(searchText) || userEmail.includes(searchText) || contentData.includes(searchText))) {
              show = false;
          }
          // Rating filter
  ...
  const applyCommentFiltersBtn = document.getElementById('applyCommentFiltersBtn');
  ...
  document.querySelectorAll('.view-btn-trigger').forEach(button => {
      button.addEventListener('click', function(e) {
          ...
          document.querySelector('.comment-post-type-detail').textContent = itemTypeDetail ? `(${itemTypeDetail})` : '';
          ...
          if (imageUrl && imageUrl !== '/images/default-postImage.png' && imageUrl.startsWith('/')) {
              imageContainer.style.display = 'block';
              modalImage.src = imageUrl;
              modalImage.alt = itemType === 'Review' ? 'Ảnh đánh giá' : 'Ảnh bình luận';
          } else {
  ...
  document.querySelectorAll('.delete-btn-trigger').forEach(button => {
      button.addEventListener('click', function(e) {
          ...
          const deleteMessage = itemToDeleteType === 'Review' 
      ...
      if (itemToDeleteId && itemToDeleteType) {
          ...
          .then(data => {
              if (data.success) {
                  ...
                  const rows = document.querySelectorAll(`[data-id="${itemToDeleteId}"]`);
                  rows.forEach(r => {
                  ...
                  if (itemToDeleteType === 'Review') {
                      const remainingReviews = reviewsTable.querySelectorAll('tbody tr:not([style*="display: none"])');
                      if (remainingReviews.length === 0) {
                          const noDataRow = document.createElement('tr');
                          noDataRow.innerHTML = '<td colspan="7" class="text-center" style="padding: 20px;">Không có đánh giá nào.</td>';
                          reviewsTable.querySelector('tbody').appendChild(noDataRow);
                      }
                  } else {
                      const remainingComments = commentsTable.querySelectorAll('tbody tr:not([style*="display: none"])');
                      if (remainingComments.length === 0) {
                          const noDataRow = document.createElement('tr');
                          noDataRow.innerHTML = '<td colspan="7" class="text-center" style="padding: 20px;">Không có bình luận nào.</td>';
                  ...
                  showToast('Có lỗi xảy ra: ' + (data.message || 'Không thể xóa mục này'), 'error');
              }
