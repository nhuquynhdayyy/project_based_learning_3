document.addEventListener("DOMContentLoaded", () => {
  ...
  if (viewButtons && userModal) {
    viewButtons.forEach((button) => {
      button.addEventListener("click", function () {
        ...
        const username = row.querySelector(".user-username").textContent.substring(1) // Remove @ symbol
        const email = row.querySelector("td:nth-child(3)").textContent
        const role = row.querySelector("td:nth-child(4) .badge").textContent

        // Set modal title and populate form
        document.querySelector(".modal-title").textContent = "Chi tiết người dùng"
  ...
  if (editButtons && userModal) {
    editButtons.forEach((button) => {
      button.addEventListener("click", function () {
        ...
        // Set modal title and populate form
        document.querySelector(".modal-title").textContent = "Chỉnh sửa người dùng"
        document.getElementById("fullname").value = name
        document.getElementById("username").value = username
        document.getElementById("email").value = email
        ...
        document.getElementById("confirm-password").closest(".form-group").style.display = "block"
  ...
  // Bulk Actions
  const bulkActionSelect = document.querySelector(".bulk-actions .select-control")
  const bulkActionButton = document.querySelector(".bulk-actions .btn-secondary")
  ...
  if (bulkActionSelect && bulkActionButton) {
    bulkActionButton.addEventListener("click", () => {
      ...
      alert(
        `Đã áp dụng hành động "${bulkActionSelect.options[bulkActionSelect.selectedIndex].text}" cho ${selectedUsers.length} người dùng!`,
      )
    })
  }
})
